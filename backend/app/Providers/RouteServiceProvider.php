...
class RouteServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        $this->routes(function () {
            Route::middleware('web')->group(function () {
                Route::group([], base_path('routes/frontend-redirects.php'));
                Route::prefix('/api')->group(base_path('routes/api-panel.php'));
                Route::group([], base_path('routes/locales.php'));
            });
            ...
            Route::middleware('module-api')
                ->prefix('/api/modules')
                ->scopeBindings()
                ->group(function () {
                    foreach (app(ModuleManager::class)->enabled() as $module) {
                        $routesPath = $module->apiRoutesPath();

                        if ($routesPath === null) {
                            continue;
                        }

                        Route::prefix('/' . $module->id())
                            ->name('api.modules.' . $module->id() . '.')
                            ->middleware(['module:' . $module->id(), 'throttle:api.modules'])
                            ->group($routesPath);
                    }
                });
        });
    ...
    protected function configureRateLimiting(): void
    {
        ...
        RateLimiter::for('authentication', function (Request $request) {
            if ($request->route()->named(['auth.post.forgot-password', 'api.auth.post.forgot-password'])) {
        ...
        RateLimiter::for('api.modules', function (Request $request) {
            $moduleId = (string) $request->attributes->get('aether.module.id', 'unknown');

            return Limit::perMinute((int) config('modules.rate_limit', 120))
                ->by($moduleId . '|' . $request->ip());
        });
