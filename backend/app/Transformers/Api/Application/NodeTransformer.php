...
class NodeTransformer extends BaseTransformer
{
    ...
    public function transform(Node $node): array
    {
        ...
        if ($node->relationLoaded('servers')) {
            $resources = $node->getRelation('servers');
            $allocatedMemory = (int) $resources->sum('memory');
            $allocatedDisk = (int) $resources->sum('disk');
        } elseif (!is_null($node->getAttribute('servers_sum_memory')) || !is_null($node->getAttribute('servers_sum_disk'))) {
            $allocatedMemory = (int) ($node->getAttribute('servers_sum_memory') ?? 0);
            $allocatedDisk = (int) ($node->getAttribute('servers_sum_disk') ?? 0);
        } else {
