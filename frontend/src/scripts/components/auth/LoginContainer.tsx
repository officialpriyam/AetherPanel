    ...
    return (
        ...
        >
            {({ isSubmitting, setSubmitting, submitForm }) => (
                <LoginFormContainer title={t('login.title')}>
                    ...
                    <div css={tw`mt-6`}>
                        ...
                        <div className={'relative'}>
                            <Field type={eyeOpen ? 'text' : 'password'} placeholder={t('login.password')} name={'password'} disabled={isSubmitting} icon={KeyIcon} className={'!pr-10'} />
                            <button type={'button'} className={'absolute top-1/2 right-3 -translate-y-1/2 rounded-component p-1 text-gray-300 transition hover:text-gray-100 focus:ring-1 focus:ring-primary-300'} onClick={() => setEyeOpen(!eyeOpen)} aria-label={eyeOpen ? 'Hide password' : 'Show password'}>
                                {eyeOpen 
