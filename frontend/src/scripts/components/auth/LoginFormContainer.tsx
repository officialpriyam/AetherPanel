...
export default forwardRef<HTMLFormElement, Props>(({ title, ...props }, ref) => {
    ...
    const copyright = useStoreState((state: ApplicationStore) => state.settings.data!.flash.copyright);
    ...
    return(
        <div className={'my-auto flex w-full flex-col items-center px-4 py-8 sm:px-6'}>
            <div className={'w-full max-w-[420px]'}>
                <FlashMessageRender css={tw`mb-3`} />
            </div>
            <Form {...props} ref={ref} className={`w-full max-w-[420px] ${props.className || ''}`}>
                <div className={'w-full rounded-box border border-gray-500/60 bg-gray-700/95 p-5 shadow-2xl shadow-black/25 backdrop-blur-md sm:p-6'}>
                    {logoPosition == 1 &&
                    <div className='flex min-w-0 gap-x-2 items-center font-semibold text-lg text-gray-50 pb-5'>
                        <img src={logo} alt={name + 'logo'} css={`height:${logoHeight};`} />
                        {String(fullLogo) === 'false' && <span className={'truncate'}>{name}</span>}
                    </div>}
                    {title && <h2 css={tw`text-lg text-gray-50 font-medium mb-3`}>{title}</h2>}
                    {props.children}

                    {socialPosition == 2 &&
                        <div className={'flex flex-wrap justify-center gap-x-6 gap-y-2 mt-5 text-sm text-gray-300'}>
            ...
            <div className={'mt-4 w-full max-w-[420px] px-2'}>
                <p css={tw`text-center text-neutral-300 text-xs`}>
                    {name} &copy; 2015 - {new Date().getFullYear()}
                </p>
                <p css={tw`text-center text-neutral-300 text-xs`}>
                    {copyright == 'Designed by priyxstudio.one' ?
                        <>
                        Designed by
