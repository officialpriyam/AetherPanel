...
export interface UserStore {
    ...
    updateUserAvatar: Thunk<UserStore, { avatarUrl: string | null }, any, UserStore, Promise<string | null>>;
}
...
const user: UserStore = {
    ...
    updateUserAvatar: thunk(async (actions, payload) => {
        const avatarUrl = await updateAccountAvatar(payload.avatarUrl);

        actions.updateUserData({ avatarUrl });
