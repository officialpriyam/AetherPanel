...
export default ({ server }: { server: Server }) => {
    const { t } = useTranslation(['flash/utilities', 'flash/dashboard']);
    ...
    return (
        <>
        <div
            className="backdrop overflow-hidden rounded-box cursor-grab active:cursor-grabbing"
            style={{ backgroundColor: 'rgba(15, 23, 42, 0.7)', border: '1px solid rgba(148, 163, 184, 0.12)' }}
            draggable={true}
            onDragStart={handleDragStart}
        >
            <div className={'bg-center bg-cover bg-no-repeat relative px-6 pt-5 z-10'} css={`background-image:url(${server.eggImage ? server.eggImage : '/flash/minecraft-banner.png'})`}>
                <div className={'z-[-1] absolute inset-0'} css={'background-image:linear-gradient(0deg, rgba(15, 23, 42, 0.92) 0%, rgba(15, 23, 42, 0.45) 100%);'}/>
